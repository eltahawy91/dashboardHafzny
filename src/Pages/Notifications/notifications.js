...
function Notifications() {
    ...
    const arrayColum = [{ "title": "العنوان" }, { "message": 'المحتوي' }, { "target_type": "الفئه المستهدفه" },  { "action": "عمليات" }]
    ...
    const rows = data && data.notifications ?
        data.notifications.map(item => createData(item.id,item.title, item.message, item.target_type , [deleteImg, editImg])) :
    [];
    ...
    return (
        <div className="apDiv notifications">
            ...
            <div className="body_container container" dir='rtl'>
                <Header title="الاشعارات" nameFunction={() => TypeSend("notifications" , "/Dashboard/notifications" )} onSearchChange={setOnSearchChange} />
                { data && data.notifications.length === 0 ? (
                    ...
                    <TableComponent arrayColum={arrayColum} rows = {rows} hight = {600} path= "/Dashboard/notifications" onSearchChange={onSearchChange} />
                }
            </div>
        </div>
    );
